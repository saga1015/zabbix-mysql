...
PATHTOCRON="/usr/local/etc/cron.d"
...
if [ $SIMULATE = 0 ]; then
	if [ $NONINTERACTIVE = 1 ]; then
	...
	else    # start here normally
		echo -e "\nReady to update permissions of Zabbix user to create routines\n"
		echo -n "Enter root DB user: "
		read DBADMINUSER
		echo -n "Enter $DBADMINUSER password: "
		read DBADMINPASS
                echo "GRANT CREATE ROUTINE ON zabbix.* TO '$DBUSER'@'localhost';" | mysql -B -h $DBHOST -u $DBADMINUSER -p$DBADMINPASS
		echo -e "\n"
		...
		if [ "$yn" != "n" -a "$yn" != "N" ]; then
			...
			#
			echo "GRANT LOCK TABLES ON zabbix.* TO '${DBUSER}'@'${DBHOST}' IDENTIFIED BY '${DBPASS}';" | mysql -h${DBHOST} -u${DBADMINUSER} -p${DBADMINPASS}

			mysqldump --opt -h ${DBHOST} -u ${DBUSER} -p${DBPASS} -d zabbix > ${DUMP_FILE}
			rc=$?
			if [ $rc -ne 0 ]; then
...
#	case $i in
#		history_log)
#			echo "ALTER TABLE $i DROP KEY history_1;" >>$SQL
#			echo "ALTER TABLE $i ADD KEY history_1(itemid, id);" >>$SQL
...
# disabled housekeeping in fronted: Administration->Housekeeping, disable History and Trends!
